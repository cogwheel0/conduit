...
class Rustup {
  ...
  void installToolchain(String toolchain) {
    ...
    final targets = _getInstalledTargets(toolchain);
    final existing = _installedToolchains.firstWhereOrNull(
      (entry) => entry.name == toolchain,
    );
    if (existing == null) {
      _installedToolchains.add(_Toolchain(toolchain, targets));
    } else {
      existing.targets
        ..clear()
        ..addAll(targets);
  ...
  /// Ensures Cargo can find the compiler selected by [toolchain].
  ///
  /// rustup installed through Homebrew can be available without installing
  /// the `rustc` proxy into `~/.cargo/bin`. `rustup run ... cargo` selects the
  /// right Cargo binary but Cargo still resolves `rustc` through PATH.
  static Map<String, String> toolchainEnvironment(String toolchain) {
    final result = runCommand(
      'rustup',
      ['which', 'rustc', '--toolchain', toolchain],
    );
    final rustcPath = result.stdout.toString().trim();
    if (rustcPath.isEmpty) {
      throw StateError('rustup returned an empty rustc path for $toolchain');
    }
    ...
    return {
      'PATH': [
        ...
        if (inheritedPath != null && inheritedPath.isNotEmpty) inheritedPath,
      ].join(separator),
    };
  }
  ...
  static List<_Toolchain> _getInstalledToolchains() {
    ...
    // Keep official channels and exact-version toolchains while ignoring
    // user-linked custom toolchains.
