...
// SAFETY: `data` is mutated only while `lock` is held. Shared access to the
// pinned value additionally requires the value itself to be safe to share.
unsafe impl<T: Sync> Sync for PagableArcInner<T> {}
...
// the struct packs into 8 usizes. Most 32-bit targets keep AtomicU64 aligned to
// 8 bytes and occupy 12 usizes. Android ARM aligns it to 4 bytes and occupies
// 10; keep the assertion while accounting for that target ABI.
...
impl<T: Pagable> PagableArcInner<T> {
        ...
        T: Pagable,
    {
        ...
        let handle = tokio::runtime::Handle::try_current()
            .map_err(|_| anyhow::anyhow!("pin_sync requires a Tokio multi-thread runtime"))?;
        if handle.runtime_flavor() != tokio::runtime::RuntimeFlavor::MultiThread {
            return Err(anyhow::anyhow!(
                "pin_sync is unsupported on a current-thread Tokio runtime"
            ));
        }
        tokio::task::block_in_place(|| {
            handle.block_on(async move { self.alloc_pinned_async().await })
        })
    }
