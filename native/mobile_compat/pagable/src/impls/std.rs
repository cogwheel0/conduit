...
impl<'de, T: PagableDeserialize<'de>> PagableBoxDeserialize<'de> for [T] {
    fn deserialize_box<D: PagableDeserializer<'de> + ?Sized>(
        ...
        // The length comes from untrusted serialized input. Reserve a bounded
        // initial chunk and grow incrementally while still decoding exactly
        // the declared number of elements.
        let mut v = Vec::with_capacity(items.min(1024));
        for _ in 0..items {
