            ...
            match task {
                Task::Start(v) => {
                    ...
                    if in_progress.contains(&identity) {
                        fail_claimed_slots(finished, &in_progress);
                        return Err(PageOutError::Failed(anyhow::anyhow!(
                            "cyclic pagable arc graph detected"
                        )));
                    }
                    let slot = finished
                        .entry(identity)
                    ...
                    if arcs.iter().any(|arc| in_progress.contains(&arc.identity())) {
                        fail_claimed_slots(finished, &in_progress);
                        return Err(PageOutError::Failed(anyhow::anyhow!(
                            "cyclic pagable arc graph detected"
                        )));
                ...
                Task::Finish((arc, data, child_arcs)) => {
                    ...
                    let slot = finished.get(&identity).expect("slot should exist").dupe();
...
fn fail_claimed_slots(finished: &DashMap<usize, Arc<ArcSerSlot>>, identities: &HashSet<usize>) {
    for identity in identities {
        let slot = finished.get(identity).map(|slot| slot.dupe());
