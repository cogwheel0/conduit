...
#[cfg(any(target_os = "android", target_os = "ios"))]
static MOBILE_INSTALLATION_ID_LOCK: tokio::sync::Mutex<()> = tokio::sync::Mutex::const_new(());
...
impl InProcessClientHandle {
    ...
    pub async fn shutdown(self) -> IoResult<()> {
        ...
        #[cfg(any(target_os = "android", target_os = "ios"))]
        let runtime_thread = self.runtime_thread;
        ...
        #[cfg(any(target_os = "android", target_os = "ios"))]
        {
            // The shutdown acknowledgement is sent only after the app-server
            // has drained its processors. Join outside Tokio's worker pool so
            // dropping the dedicated runtime cannot block unrelated FRB work.
            let _ = tokio::task::spawn_blocking(move || runtime_thread.join()).await;
        }
        Ok(())
    }
...
async fn resolve_runtime_installation_id(codex_home: &AbsolutePathBuf) -> IoResult<String> {
    match resolve_installation_id(codex_home).await {
        Ok(installation_id) => Ok(installation_id),
        #[cfg(any(target_os = "android", target_os = "ios"))]
        Err(error) if error.kind() == ErrorKind::Unsupported => {
            warn!("advisory installation-id locking is unsupported; using mobile fallback");
            resolve_mobile_installation_id(codex_home).await
        }
        Err(error) => Err(error),
    }
}
...
#[cfg(any(target_os = "android", target_os = "ios"))]
async fn resolve_mobile_installation_id(codex_home: &AbsolutePathBuf) -> IoResult<String> {
    // A Conduit mobile runtime is in-process and single-account. This mutex
    // preserves same-process serialization when the app-private filesystem
    // rejects Codex's advisory cross-process file lock.
    let _guard = MOBILE_INSTALLATION_ID_LOCK.lock().await;
    ...
    tokio::fs::write(path, installation_id.as_bytes()).await?;
    Ok(installation_id)
}

async fn start_uninitialized(args: InProcessStartArgs) -> IoResult<InProcessClientHandle> {
    let channel_capacity = args.channel_capacity.max(1);
    let installation_id = resolve_runtime_installation_id(&args.config.codex_home).await?;
    ...
    #[cfg(not(any(target_os = "android", target_os = "ios")))]
    let runtime_handle = tokio::spawn(runtime_future);
    #[cfg(any(target_os = "android", target_os = "ios"))]
    let runtime_thread = spawn_mobile_runtime(runtime_future)?;
...
#[cfg(any(target_os = "android", target_os = "ios"))]
fn spawn_mobile_runtime<F>(runtime_future: F) -> IoResult<std::thread::JoinHandle<()>>
where
    F: Future<Output = ()> + Send + 'static,
{
    let runtime = tokio::runtime::Builder::new_multi_thread()
        .worker_threads(2)
        .thread_name("codex-mobile-runtime")
        .thread_stack_size(MOBILE_RUNTIME_STACK_SIZE_BYTES)
        .enable_all()
        .build()
        .map_err(|error| IoError::other(format!("failed to build mobile runtime: {error}")))?;

    std::thread::Builder::new()
        .name("codex-mobile-runtime".to_owned())
        .stack_size(MOBILE_RUNTIME_STACK_SIZE_BYTES)
        .spawn(move || runtime.block_on(Box::pin(runtime_future)))
        .map_err(|error| IoError::other(format!("failed to start mobile runtime: {error}")))
}
