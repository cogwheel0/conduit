...
impl MessageProcessor {
    ...
    async fn handle_initialized_client_request(
        ...
        // Keep the mobile future deliberately small. The complete desktop
        // dispatcher captures every coding/workspace request branch in one
        // async state machine and exceeds Android's worker-thread stack before
        // its first poll. Conduit exposes only these chat/account operations.
        let result: Result<Option<ClientResponsePayload>, JSONRPCErrorError> = match codex_request {
            ...
            _ => Err(invalid_request(
                "request is outside the Conduit mobile-chat capability boundary",
            )),
        };
